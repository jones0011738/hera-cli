...
- **All limits removed** — no artificial execution or performance caps unless the user explicitly sets them:
  ...
  - `HERA_CONTEXT_TOKENS` default `131072` (was `32000`). Server is now `--ctx-size 262144 --parallel 2` = **131072 tokens per slot**; default matches exactly so auto-compaction fires before the slot fills.
